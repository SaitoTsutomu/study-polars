...
def proc_prob(fp_ok, fp_ng, count, cell1, cell2, cell3, cell4):  # noqa: C901 PLR0912 PLR0913 PLR0917
    msg = f"Line {count}: invalid 問題"
    source = cell1["source"]
    ...
    msg = f"Line {count + 1}: invalid 解答欄"
    source = cell2["source"]
    ...
    msg = f"Line {count + 2}: invalid 解答例"
    source = cell3["source"]
    ...
    while count < n_cells:
        ...
        if cell_type == "markdown":
            if source.startswith("### `問題"):
                ...
                msg = f"Line {count}: invalid 解答例"
                raise ValueError(msg)
        elif cell_type == "code":
            last = source.strip().splitlines()[-1]
            if last.startswith("# ここから解答を作成"):
                msg = f"Line {count}: invalid 解答欄"
                raise ValueError(msg)
